...
collection PublicKeyArrayDemo {
    ...
    struct PublicKeyArrayDemo {
        id: String,
        keys: Vec<indexer_rocksdb::PublicKey>,
    }
    ...
    let public_key1 = indexer_rocksdb::PublicKey::from_secp256k1_key(&public_key1).unwrap();

    let (_private_key2, public_key2) = secp256k1::generate_keypair(&mut rand::thread_rng());
    let public_key2 = indexer_rocksdb::PublicKey::from_secp256k1_key(&public_key2).unwrap();
    ...
    let public_key3 = indexer_rocksdb::PublicKey::from_secp256k1_key(&public_key3).unwrap();
